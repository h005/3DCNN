...
public:
    ...
    // Find the optimal seams with max energy
    vector<uint> findVerticalMaxSeam();
    vector<uint> findHorizontalMaxSeam();
    ...
    //Dispolay a given seam on a given image
    void showHorizontalSeam(vector<uint> seam, Mat &img, fstream &fout);
};
