...
void SeamCarver::showHorizontalSeam(vector<uint> seam, Mat &img, fstream &fout)
{

    for (int i=0; i < img.cols; ++i)
    {
        img.at<Vec3b>(seam[i],i) = Vec3b(255,255,255); //Set the color of the seam to Red
//        printf("img%06d.jpg\n",seam[i] + i * 32 + 1);
        fout.width(6);
        fout.fill('0');
...
vector<uint> SeamCarver::findVerticalMaxSeam()
{
    vector<uint> seam(image.rows);
    ...
    for (int i=0; i< image.rows; i++)
    {
        for (int j=0; j< image.cols; j++)
        {
            if( i==0 )
                ...
                distTo[i][j] = numeric_limits<unsigned int>::min();
            edgeTo[i][j] = 0;
        }
    }
...
vector<uint> SeamCarver::findHorizontalMaxSeam()
{
    vector<uint> seam(image.cols);
    // Transpose the matrices and find the vertical seam
    transpose(image, image);
